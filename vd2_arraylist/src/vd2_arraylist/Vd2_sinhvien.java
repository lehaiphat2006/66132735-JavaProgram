...
public class Vd2_sinhvien {
	...
	// Lớp SinhVien
	    ...
	    public Vd2_sinhvien(String maSV, String hoTen, double diemTB) {
	        this.maSV = maSV;
	        this.hoTen = hoTen;
	        this.diemTB = diemTB;
	    }
...
//Lớp quản lý sinh viên sử dụng ArrayList
 ...
 private List<Vd2_sinhvien> danhSachSV;
 ...
 // Thêm sinh viên - add()
 public void themSinhVien(Vd2_sinhvien sv) {
     danhSachSV.add(sv);
     System.out.println("Đã thêm: " + sv.getHoTen());
 }
 
 // Tìm sinh viên theo mã - sử dụng get() và vòng lặp
 public Vd2_sinhvien timTheoMa(String maSV) {
     for (Vd2_sinhvien sv : danhSachSV) {
 ...
 public Vd2_sinhvien timDiemCaoNhat() {
     if (danhSachSV.isEmpty()) {
         return null;
     }
     
     Vd2_sinhvien svCaoNhat = danhSachSV.get(0);
     for (Vd2_sinhvien sv : danhSachSV) {
 ...
 public List<Vd2_sinhvien> locSinhVienTheoDiem(double diemToiThieu) {
     List<Vd2_sinhvien> ketQua = new ArrayList<>();
     for (Vd2_sinhvien sv : danhSachSV) {
     ...
     for (int i = 0; i < danhSachSV.size() - 1; i++) {
         for (int j = 0; j < danhSachSV.size() - i - 1; j++) {
             if (danhSachSV.get(j).getDiemTB() < danhSachSV.get(j + 1).getDiemTB()) {
                 // Hoán đổi
                 Vd2_sinhvien temp = danhSachSV.get(j);
 ...
 public void hienThiDanhSach() {
     ...
     for (int i = 0; i < danhSachSV.size(); i++) {
         Vd2_sinhvien sv = danhSachSV.get(i);
 ...
 public boolean xoaSinhVien(String maSV) {
     for (int i = 0; i < danhSachSV.size(); i++) {
         if (danhSachSV.get(i).getMaSV().equals(maSV)) {
             Vd2_sinhvien svBiXoa = danhSachSV.remove(i);
 ...
 public void hienThiHaiSinhVienDauTien() {
     if (danhSachSV.size() >= 2) {
         List<Vd2_sinhvien> subList = danhSachSV.subList(0, 2);
...
class MainQuanLySinhVien {
  public static void main(String[] args) {
      ...
      // Thêm sinh viên
      qlsv.themSinhVien(new Vd2_sinhvien("SV001", "Nguyễn Văn A", 8.5));
      qlsv.themSinhVien(new Vd2_sinhvien("SV002", "Trần Thị B", 7.2));
      qlsv.themSinhVien(new Vd2_sinhvien("SV003", "Lê Văn C", 6.8));
      qlsv.themSinhVien(new Vd2_sinhvien("SV004", "Phạm Thị D", 9.1));
      qlsv.themSinhVien(new Vd2_sinhvien("SV005", "Hoàng Văn E", 5.5));
      ...
      // Tìm sinh viên
      System.out.println("\n=== TÌM KIẾM SINH VIÊN ===");
      Vd2_sinhvien sv = qlsv.timTheoMa("SV003");
      System.out.println("Tìm SV003: " + (sv != null ? sv : "Không tìm thấy"));
      
      // Tìm điểm cao nhất
      System.out.println("\n=== SINH VIÊN ĐIỂM CAO NHẤT ===");
      Vd2_sinhvien svCaoNhat = qlsv.timDiemCaoNhat();
      ...
      List<Vd2_sinhvien> svGioi = qlsv.locSinhVienTheoDiem(8.0);
      if (!svGioi.isEmpty()) {
